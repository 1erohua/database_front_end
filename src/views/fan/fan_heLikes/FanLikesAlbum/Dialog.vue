...
<script>
import { addLikedAlbum, updateLikedAlbum } from "@/api/request";
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.albumInfo.AlbumID) {
            // 更新专辑信息
            updateLikedAlbum(this.$store.state.fanId, this.albumInfo.AlbumID, this.albumInfo)
              .then(() => {
            ...
            // 新增专辑
            addLikedAlbum(this.$store.state.fanId, this.albumInfo)
              .then(() => {
