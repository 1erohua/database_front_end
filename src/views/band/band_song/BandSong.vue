...
<script>
import { getBandSongs, deleteBandSong } from "@/api/request";
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.$confirm("确认删除？")
        .then(() => {
          const bandId = this.$store.state.bandId;
          deleteBandSong(bandId, row.SongID)
            .then(() => {
              this.$message({
                type: "success",
                message: "删除成功!",
      ...
      this.$confirm("确认删除选中？")
        .then(() => {
          ...
          this.selectedData.forEach(async (item, index) => {
            await deleteBandSong(bandId, item.SongID);
            this.$message({
              type: "info",
